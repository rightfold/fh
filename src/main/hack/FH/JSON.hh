...
final class JSON {
    ...
    public static function dateTime(?DateTimeInterface $v): JSON {
        return new JSON($v === null ? null : $v->format(DateTime::ISO8601));
    }

    public static function array_(?ImmVector<JSON> $vs): JSON {
        return new JSON($vs === null ? null : $vs->map(function($v) { return $v->value; })->toArray());
    }

    public static function object(?ImmMap<string, JSON> $vs): JSON {
        return new JSON($vs === null ? null : (object)$vs->map(function($v) { return $v->value; })->toArray());
    }
